function RecipeTab({ recipes, setRecipes, cubes, recipeStatus, dishes, stock, unitRecipes }) {
  ...
  const emptyForm = { name:"", color:"#FFB347", note:"", ingredients:[], unitIds:[], dishId:"", slotMap:{}, slotUnits:{} };
  ...
  const openEdit = r => { setEditId(r.id); setForm({name:r.name,color:r.color,note:r.note||"",ingredients:r.ingredients.map(x=>({...x})),unitIds:r.unitIds||[],dishId:r.dishId||"",slotMap:r.slotMap||{},slotUnits:r.slotUnits||{}}); setModal(true); };
  const openCopy = r => { setEditId(null); const newIngs=r.ingredients.map(x=>({...x})); setForm({name:r.name+" (복사)",color:r.color,note:r.note||"",ingredients:newIngs,unitIds:r.unitIds||[],dishId:r.dishId||"",slotMap:rebuildSlotMap(r.slotMap,newIngs)}); setModal(true); };
  ...
  const save = () => {
    ...
    if (editId) {
      setRecipes(rs => rs.map(r => r.id===editId ? {...r,name:form.name,color:form.color,note:form.note,ingredients:ings,unitIds:form.unitIds||[],dishId:form.dishId,slotMap:cleanSlotMap,slotUnits:form.slotUnits||{},updatedAt:todayStr()} : r));
    } else {
      setRecipes(rs => [...rs, {id:uid(),name:form.name,color:form.color,note:form.note,ingredients:ings,unitIds:form.unitIds||[],dishId:form.dishId,slotMap:cleanSlotMap,slotUnits:form.slotUnits||{},updatedAt:todayStr()}]);
    }
    setModal(false);
  };
  ...
  const sorted = [...recipes].sort((a,b) => {
    ...
    if (sortBy==="vol_desc") return cubeVolume(b,cubes,unitRecipes)-cubeVolume(a,cubes,unitRecipes);
    if (sortBy==="vol_asc")  return cubeVolume(a,cubes,unitRecipes)-cubeVolume(b,cubes,unitRecipes);
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <span style={{fontWeight:700,fontSize:16,color:"#333"}}>레시피 목록 ({recipes.length})</span>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          <PillBtn onClick={openNew} small>+ 레시피 추가</PillBtn>
        </div>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {sorted.map(r => {
          ...
          const vol = cubeVolume(r, cubes, unitRecipes);
                  ...
                  {(r.unitIds||[]).length > 0 && (
                    <div style={{marginBottom:6,display:"flex",flexWrap:"wrap",gap:4}}>
                      {(r.unitIds||[]).map(uId=>{
                        ...
                        if(!u) return null;
                        return <span key={uId} style={{background:u.color+"22",border:"1.5px solid "+u.color+"88",borderRadius:20,padding:"2px 8px",fontSize:11,fontWeight:600,color:"#444"}}>🍱 {u.name}</span>;
                      })}
                    </div>
                  )}
      ...
      <Overlay open={modal} onClose={()=>setModal(false)} title={editId?"레시피 수정":"새 레시피 추가"}>
        <Field label="레시피 이름" value={form.name} onChange={v=>setForm(f=>({...f,name:v}))} placeholder="단호박 죽" />
        ...
        {unitRecipes && unitRecipes.length > 0 && (
          <div style={{marginBottom:14}}>
            <div style={{fontSize:12,color:"#888",marginBottom:8,fontWeight:600}}>🍱 유닛 레시피 구성</div>
            {/* 선택된 유닛 목록 */}
        ...
        <div style={{marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div style={{fontSize:12,color:"#888"}}>재료 & 큐브 수 <span style={{fontSize:10,color:"#bbb"}}>(유닛 외 추가 재료)</span></div>
                        ...
                        <div key={slot} style={{borderRadius:10,border:"2px solid "+SLOT_COLORS[si],background:"#fff",padding:"8px 6px"}}>
                          ...
                          {unitRecipes && unitRecipes.length > 0 && (
                            <div style={{marginBottom:6}}>
                              <select
                                ...
                                style={{width:"100%",fontSize:9,padding:"2px 4px",borderRadius:6,border:"1px solid #ddd",
                                  background:(form.slotUnits||{})[slot]
                                    ? (unitRecipes.find(u=>u.id===(form.slotUnits||{})[slot])?.color||"#fff")+"33"
                              ...
                              {(form.slotUnits||{})[slot] && (()=>{
                                const su = unitRecipes.find(u=>u.id===(form.slotUnits||{})[slot]);
                                if(!su) return null;
      ...
      <ConfirmDelete
        open={!!confirmDel}
        message={confirmDel ? `"${confirmDel.name}" 레시피를 삭제할까요?` : ""}
